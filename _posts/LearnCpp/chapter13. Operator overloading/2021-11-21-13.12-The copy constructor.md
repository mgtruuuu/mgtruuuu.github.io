...
// Uniform initialization of a Fraction, calls Fraction(int, int) constructor
Fraction fiveThirds {5, 3};
```
...
int main() {
    ...
    // Copy constructor is private, compile error on this line
    Fraction fCopy{ fiveThirds }; 
...
As of `C++17`, some cases of copy elision (including the example above) have been made mandatory. *In these mandatory elision cases, the copy constructor does not need to accessible (or even present)* since it is guaranteed not to be needed!