...
Container classes generally come in two different varieties. **Value containers** are [compositions](https://www.learncpp.com/cpp-tutorial/composition/) that store copies of the objects that they are holding (and thus are responsible for creating and destroying those copies). **Reference containers** are [aggregations](https://www.learncpp.com/cpp-tutorial/103-aggregation/) that store pointers or references to other objects (and thus are not responsible for creation or destruction of those objects).
...
~IntArray() {
    ...
    // We don't need to set m_data to null 
    // or m_length to 0 here,
...
void erase() {
    ...
    // otherwise it will be left pointing at deallocated memory!
    m_data = nullptr;
    m_length = 0;
}
```
...
void resize(int newLength) {
    ...
    //  First we are going to allocate a new array.
    //  Then we are going to copy elements
    //  from the existing array to the new array.
    //  Once that is done, we can destroy the old array,
    //  and make m_data point to the new array.

    // First we have to allocate a new array.
    int* data{ new int[newLength] };

    // Then we have to figure out how many elements to copy
    // from the existing array to the new array.
    ...
    // Note that this simply makes m_data point to the same address
    // as the new array we dynamically allocated.
    // Because data was dynamically allocated,
...
void insertBefore(int value, int index) {
    ...
    // one element larger than the old array.
...
void remove(int index) {
    ...
    // set the array to empty and bail out.
    if (m_length == 1) {
        erase();
    ...
    // First create a new array 
    // one element smaller than the old array.
    int* data{ new int[m_length - 1] };
