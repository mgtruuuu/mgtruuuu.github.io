...
tag : 
    ...
    - reference to const value, const reference
toc: true  
toc_sticky: true 
use_math : true
---
...
// ref is a reference to a const value.
...
// Create const reference to variable apples.
const int& ref{ apples }; 
...
int somefcn() {
    ...
    const int& ref{ 2 + 3 };
    // But because the result is now bound
    // to a reference to a const value...
...
```c++
// ref is a const reference to the argument
