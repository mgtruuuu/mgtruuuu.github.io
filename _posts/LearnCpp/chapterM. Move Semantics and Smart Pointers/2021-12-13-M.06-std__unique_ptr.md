...
int main() {
    ...
    //takeOwnership(ptr);           // This doesn't work,
...
There are two easy ways to misuse `std::unique_ptr`s, both of which are easily avoided. First, don’t let multiple classes manage the same resource. For example:
