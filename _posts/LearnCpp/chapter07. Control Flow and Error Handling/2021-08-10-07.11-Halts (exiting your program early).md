...
1. First, because we’re leaving the function, all local variables and function parameters are destroyed (as per usual).
2. Next, **a special function called `std::exit()` is called, with the return value from `main()` (the status code) passed in as an argument**. So what is `std::exit()`?
...
1. First, objects with static storage duration are destroyed.
2. Then some other miscellaneous file cleanup is done if any files were used.
3. Finally, control is returned back to the OS, with the argument passed to `std::exit()` used as the status code.
...
int main() {
    ...
    // Terminate and return status code 0
    // to operating system.
    std::exit(0); 
...
>**Related content**  
We discuss passing functions as arguments in [lesson 11.7 -- Function Pointers](https://www.learncpp.com/cpp-tutorial/function-pointers/).
...
int main() {
    ...
    // Register cleanup() to be called automatically 
    // when std::exit() is called.
    std::atexit(cleanup);
    // note: we use cleanup rather than cleanup()
    //       since we're not making a function call 
    //       to cleanup() right now
...
>>>How does the relationship between `std::atexit()` and `std::exit()` is made???
```c++
#include <cstdlib>  // for std::exit()
#include <iostream>
//
