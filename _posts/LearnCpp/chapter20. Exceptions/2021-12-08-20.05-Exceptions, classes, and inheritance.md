...
This is part of the reason that RAII (covered in [lesson 12.9 -- Destructors](https://www.learncpp.com/cpp-tutorial/destructors/)) is advocated so highly -- even in exceptional circumstances, classes that implement RAII are able to clean up after themselves.
...
int main() {
    ...
    // and all the derived exceptions too.
...
public:
    ...
    // std::exception::what() returns a const char*, 
    // so we must as well.
...
int main() {
    ...
    // Derived catch blocks go first.
        ...
        std::cerr << "Some other std::exception occurred ("
            << exception.what() << ")\n";
    }
}
```
